 ...
 * 
 * For example, next' `toBuffer()` function is defined as:
 * 
 * ```
...
export class SQLFragment<RunResult = pg.QueryResult['rows'], Constraint = never> {
   ...
   * value '_next_prepared_N', where N is an increasing sequence number.
   */
  prepared = (name = `_next_prepared_${preparedNameSeq++}`) => {
  ...
  run = async (queryable: Queryable, force = false): Promise<RunResult> => {
    const
      query = this.compile(),
      config = getConfig(),
      txnId = (queryable as any)._next?.txnId;
